...
public class ClientProxyWorld extends CommonProxyWorld {
    ...
    private void registerBlockItem(Block toRegister) {
        ...
        Minecraft.getMinecraft().getRenderItem().getItemModelMesher().register(
                item,
                0,
                new ModelResourceLocation(ValkyrienSkiesWorld.MOD_ID + ":" + item.getTranslationKey().substring(5), "inventory")
        );
    }
    ...
    private void registerItemModel(Item toRegister) {
        Minecraft.getMinecraft().getRenderItem().getItemModelMesher().register(
                toRegister,
                0,
                new ModelResourceLocation(ValkyrienSkiesWorld.MOD_ID + ":" + toRegister.getTranslationKey().substring(5), "inventory")
        );
    }
}
